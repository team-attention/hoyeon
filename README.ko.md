...
체인은 방향성이 있다: **요구사항이 태스크를 생성하지, 그 반대는 절대 아니다.** 요구사항이 바뀌면 하위 요구사항과 태스크는 재도출된다. Hoyeon이 실행 중 블로커를 만나도 복구할 수 있는 이유가 바로 이것이다 — 요구사항은 여전히 유효하고, 태스크만 조정하면 된다.
...
> *대부분의 AI 도구는 매 세션을 백지에서 시작한다. Hoyeon은 기억한다.*
