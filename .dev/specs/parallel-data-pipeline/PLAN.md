...
*Functional:*
- [ ] `test -f .playground/data-pipeline/data/sample.csv` → exit 0
- [ ] `test -f .playground/data-pipeline/data/sample.json` → exit 0
- [ ] `jq '.' .playground/data-pipeline/data/sample.json` → valid JSON
- [ ] `wc -l < .playground/data-pipeline/data/sample.csv` → 11 (header + 10 rows)

*Static:*
- [ ] CSV 첫 행이 헤더: `head -1 .playground/data-pipeline/data/sample.csv` → `id,name,department,score`
...
*Functional:*
- [ ] `test -x .playground/data-pipeline/scripts/fetch-csv.sh` → executable
- [ ] `cd .playground/data-pipeline && ./scripts/fetch-csv.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/output/csv-result.json` → exit 0
- [ ] `jq '.' .playground/data-pipeline/output/csv-result.json` → valid JSON

*Static:*
- [ ] `head -1 .playground/data-pipeline/scripts/fetch-csv.sh` → `#!/usr/bin/env bash`
- [ ] `grep -q 'set -euo pipefail' .playground/data-pipeline/scripts/fetch-csv.sh` → exit 0
...
*Functional:*
- [ ] `test -x .playground/data-pipeline/scripts/fetch-json.sh` → executable
- [ ] `cd .playground/data-pipeline && ./scripts/fetch-json.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/output/json-result.json` → exit 0
- [ ] `jq '.' .playground/data-pipeline/output/json-result.json` → valid JSON

*Static:*
- [ ] `head -1 .playground/data-pipeline/scripts/fetch-json.sh` → `#!/usr/bin/env bash`
- [ ] `grep -q 'set -euo pipefail' .playground/data-pipeline/scripts/fetch-json.sh` → exit 0
...
*Functional:*
- [ ] `test -x .playground/data-pipeline/scripts/fetch-api.sh` → executable
- [ ] `cd .playground/data-pipeline && ./scripts/fetch-api.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/output/api-result.json` → exit 0
- [ ] `jq '.' .playground/data-pipeline/output/api-result.json` → valid JSON

*Static:*
- [ ] `head -1 .playground/data-pipeline/scripts/fetch-api.sh` → `#!/usr/bin/env bash`
- [ ] `grep -q 'set -euo pipefail' .playground/data-pipeline/scripts/fetch-api.sh` → exit 0
- [ ] `! grep -q 'curl\|wget' .playground/data-pipeline/scripts/fetch-api.sh` → no network calls
...
*Functional:*
- [ ] `test -x .playground/data-pipeline/scripts/merge-report.sh` → executable
- [ ] `cd .playground/data-pipeline && ./scripts/merge-report.sh` → exit 0 (3개 input 파일 존재 시)
- [ ] `test -f .playground/data-pipeline/output/report.json` → exit 0
- [ ] `jq '.csv_summary' .playground/data-pipeline/output/report.json` → not null
- [ ] `jq '.json_summary' .playground/data-pipeline/output/report.json` → not null
- [ ] `jq '.api_summary' .playground/data-pipeline/output/report.json` → not null

*Static:*
- [ ] `head -1 .playground/data-pipeline/scripts/merge-report.sh` → `#!/usr/bin/env bash`
- [ ] `grep -q 'set -euo pipefail' .playground/data-pipeline/scripts/merge-report.sh` → exit 0
...
*Functional:*
- [ ] `test -x .playground/data-pipeline/run-pipeline.sh` → executable
- [ ] `cd .playground/data-pipeline && ./run-pipeline.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/output/report.json` → exit 0 (end-to-end)

*Static:*
- [ ] `head -1 .playground/data-pipeline/run-pipeline.sh` → `#!/usr/bin/env bash`
- [ ] `grep -q 'set -euo pipefail' .playground/data-pipeline/run-pipeline.sh` → exit 0
- [ ] `grep -q 'wait' .playground/data-pipeline/run-pipeline.sh` → uses parallel wait
...
*Functional:*
- [ ] `test -f .playground/data-pipeline/run-pipeline.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/scripts/fetch-csv.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/scripts/fetch-json.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/scripts/fetch-api.sh` → exit 0
- [ ] `test -f .playground/data-pipeline/scripts/merge-report.sh` → exit 0
- [ ] `cd .playground/data-pipeline && ./run-pipeline.sh` → exit 0
- [ ] `jq '.' .playground/data-pipeline/output/report.json` → valid JSON
- [ ] `jq '.csv_summary' .playground/data-pipeline/output/report.json` → not null
- [ ] `jq '.json_summary' .playground/data-pipeline/output/report.json` → not null
- [ ] `jq '.api_summary' .playground/data-pipeline/output/report.json` → not null

*Static:*
- [ ] 모든 .sh 파일에 `set -euo pipefail` 포함
- [ ] `! grep -r 'curl\|wget' .playground/data-pipeline/scripts/` → no network calls

*Runtime:*
- [ ] End-to-end pipeline 실행 성공
