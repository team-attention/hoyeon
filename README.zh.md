...
- **CLI 强制结构** — `hoyeon-cli2` 验证计划结构和任务状态转换。字段名、类型、必需关系——全部在 LLM 看到数据之前由程序检查。CLI 不是建议结构，而是**拒绝**无效结构。
...
`hoyeon-cli2` 管理 plan.json 验证和任务状态:

```bash
hoyeon-cli2 plan get <task-id> <plan-path>                    # 获取任务详情
hoyeon-cli2 plan status <task-id> <plan-path> --status done   # 更新任务状态
```
