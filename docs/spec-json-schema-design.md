...
4. **제약 사항이 명확해야 한다** — 금지/보존/범위를 타입으로 구분
...
```
spec.json
├── meta          — 이게 뭔가? (identity + provenance)
...
| Must NOT Do (글로벌) | `constraints[]` (typed) | must_not_do/preserve/scope_boundary |
...
| `scope_boundary` | 물리적 범위 제한 | "허용 경로 외 파일 수정 금지" |
...
- `scope_boundary` constraint의 `allowed_paths`는 task의 `file_scope`와 교차 검증 필수
...
### constraint.scope_boundary 확장

```json
{
  "type": "scope_boundary",
  "rule": "허용 경로 외 파일 수정 금지",
  "allowed_paths": ["scripts/", ".claude/settings.json", "dev-cli/"],
  "verified_by": "machine",
  "verify": {
    "type": "command",
    "run": "git diff --name-only | grep -vE '^(scripts/|.claude/settings.json|dev-cli/)' | wc -l",
...
| scope_boundary | rule만 | `allowed_paths[]` 추가 | machine-check 가능 |
...
- scope_boundary vs file_scope 충돌 → allowed_paths + 교차 검증
