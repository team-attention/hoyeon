...
一行のコードも書かれる前に、要件は複数の角度から精緻化されます。インタビュアーが前提を問い、ギャップアナライザーが欠落を発見し、UX レビュアーがユーザーへの影響を確認し、トレードオフアナライザーが代替案を比較検討します。それぞれの視点が要件を研ぎ澄まし、検証可能なシナリオを生成できるほど正確なものにします。

チェーンには方向性があります：**要件がタスクを生み出すのであり、その逆ではありません。** 要件が変更されれば、シナリオとタスクは再導出されます。これが、Hoyeon が実行中のブロッカーから回復できる理由です — 要件は依然として有効であり、調整が必要なのはタスクだけだからです。
...
- **契約としての導出チェーン** — Goal → Decisions → Requirements → Scenarios → Tasks はリンクされています。各レイヤーはその上のレイヤーを参照します。シナリオは要件にトレースされます。タスクはシナリオにトレースされます。チェーンが切れれば、ゲートがブロックします。つまり：**有効な要件があれば、システムは結果を生み出します** — LLM の個々の出力が変動しても、決定論的にルーティングされます。
...
`spec.json` のすべてのシナリオには `verified_by` 分類が付いています：
...
システムはすべてを `machine` 検証に向けて推進します。AC Quality Gate が各シナリオをレビューし、可能な場合は `human` 項目を `machine` に変換することを提案します。マルチモデルコードレビュー（Codex + Gemini + Claude）が独立して実行され、合意された判定を合成します。独立した検証者が隔離されたコンテキストで Definition of Done をチェックし、自己検証バイアスを排除します。

人間のレビューは、機械が真に判断できないものに限定されます — UX の感触、ビジネスロジックの正確性、命名の決定。それ以外はすべて、毎回自動的に実行されます。

---

これらは理想論ではありません。アーキテクチャによって強制されています — CLI は無効な仕様を拒否し、ゲートは未検証のレイヤーをブロックし、フックは書き込みを保護し、エージェントは隔離された環境で検証します。**正しいことをすることが最も抵抗の少ない道になる**ようにシステムは設計されています。
...
エビデンスの連鎖：**requirement → scenario → verify command → pass/fail**。意図から証明まで。
...
| AC Quality Gate | 受け入れ基準のバリデーション（反復的、最大5ラウンド） |
...
| コマンド | 説明 |
|---------|--------------|
| `/specify` | レイヤーベースのインタビュー → spec.json の導出（L0→L5）、ゲートキーパー付き |
...
- **導出チェーン** — L0→L5、各遷移にマージチェックポイント + ゲートキーパーチーム
