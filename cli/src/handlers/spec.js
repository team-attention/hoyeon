...
async function handleInit(args) {
  ...
  // --schema flag: v2 is the only supported (and default) schema. v1 is rejected.
...
/**
 * Humanize AJV error messages for specific v2 constraints.
 * Currently: composes minItems violations get a clearer message.
 */
...
/**
 * v2 semantic checks (run only after AJV schema pass).
 * (a) Every requirements[].sub[] must have non-empty given/when/then, not 'TBD'.
 * (b) Every verification.journeys[].composes[] entry must resolve to an existing sub id.
 *
 * @returns {Array<{path: string, kind: string, message: string}>}
 */
      ...
      for (const field of GWT_FIELDS) {
        ...
        if (missing || isTbd) {
          errors.push({
            ...
            kind: 'gwt-missing',
            message: missing
              ? `sub-requirement is missing '${field}' (must be a non-empty string)`
              : `sub-requirement '${field}' is 'TBD' (must be filled in before validation passes)`,
          });
        }
      }
    }
...
async function handleValidate(args) {
  ...
  if (!valid) {
    ...
    for (const e of validate.errors) {
      const path = e.instancePath || '(root)';
      process.stderr.write(`  ${path}: ${humanizeAjvMessage(e, data)}\n`);
    }
    printGuideHints(validate.errors);
    process.exit(1);
  }

  // --- Phase 1.5: v2 semantic checks (GWT presence, journey ref resolution) ---
